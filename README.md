...
`query_data.py`: demonstrates how to query the database to get metrics and reports users want to track their progress

`db_session.py`: creates a SQLAlchemy Session using a the predefined `init_db` engine. It ensures consistent session configuration across all database interactions and allows control over the session scope, which can be adjusted to either use a new session for each request or share a session across multiple requests. This is used in `insert_data.py`.

`schema.png`: a visual representation of my database schema created from `schema.dot`

`tests.py`: runs a unit test suite for each model, verifying CRUD operations and constraint verification. 
<!-- TODO: anything else? -->
...
2. Composite indexes
Looking at my queries, it is clear I frequently want to limit my search by time frame -- "how many for user 1 in the last week" etc. This could be expensive to look up because even if the user_id is indexed a log running database will have a huge number of entries. Finding the correct date is expensive. Thus, for the tables holding both a user id and a date, I added a composite index (beginning with user id to be most efficient).
...
Overall, as the database grows, it will be important to keep testing if the storage space and write operations are too taxed by maintaining these index. 
